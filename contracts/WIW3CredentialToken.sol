...
contract WIW3CredentialToken is ERC721, ERC721URIStorage, Ownable {
    ...
    function mintBase(address to) public onlyOwner {
        ...
        console.log("Done! Here is the minted Credential Token: ", tokenId);
    }
        ...
        returns (CredentialToken[] memory)
    {
        ...
        CredentialToken[] memory tokens = new CredentialToken[](tokenCount);
        for (uint256 i = 0; i < tokenCount; i++) {
            uint256 currentId = i + 1;
            CredentialToken memory currentToken = credentialTokens[currentId];
    ...
    function updateLevel(uint256 tokenId, uint256 currentLevel) public {
	   ...
	   so our modifications will persist. */
        ...
        if (currentLevel == 0) {
            // Test:
            console.log("Updating Token URI with ", "Learn", currentLevel);
        ...
        if (currentLevel == 1) {
            // Testing:
            console.log("Updating Token URI with ", "Prototype", currentLevel);
        ...
        if (currentLevel == 2) {
            // Testing:
            console.log("Updating Token URI with ", "Venture", currentLevel);
